...
function handleVideo() {
  ...
  const openVideo = (e) => {
    ...
    videoNumber = e.target.dataset.video;
    videofile = e.target.dataset.videoid;
    let videoContainerPlay = document.querySelector(`#video${videoNumber}`);
    videoContainerPlay.classList.add('playing');

    setTimeout(() => {
      videoContainerPlay.querySelector('video').play();
    }, 1000);
  ...
  const closeVideo = () => {
    ...
    let videoContainerClose = document.querySelector('.playing');
    videoContainerClose.querySelector('video').pause();
    videoContainerClose.classList.remove('playing');
